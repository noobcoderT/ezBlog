...
def create_blueprint(import_name, app, storage):
    ...
    def upload():
        ...
        if len(f) > 0:
            try:
                fname = request.files[f].filename
                fdata = request.files[f]
                filename = storage.store(fname, fdata)
            except (StorageNotAllowed, StorageExists) as e:
                ret = {"success":0, "message":str(e), "url":""}
                return jsonify(ret)

            url = '/static/upload/'+filename
            ret = {"success":1, "message":"upload done", "url":url}
            return jsonify(ret)
